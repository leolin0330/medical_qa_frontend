...
class _NewsPageState extends State<NewsPage> with AutomaticKeepAliveClientMixin {
  ...
  void initState() {
    ...
    _future = _api.fetchNews(source: 'who', limit: 10, lang: 'zh-TW');
  }
          ...
          builder: (context, snap) {
            ...
            return ListView.separated(
              ...
              itemBuilder: (context, i) {
                ...
                final title   = ((it['title_zh'] ?? it['title']) ?? '').toString().trim();
                final date = (it['published'] as String? ?? '').trim();
                // final summary = (it['summary'] as String? ?? '').trim();
                final summary = ((it['summary_zh'] ?? it['summary']) ?? '').toString().trim();
