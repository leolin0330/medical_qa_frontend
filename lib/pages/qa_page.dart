...
class _QaPageState extends State<QaPage> {
  ...
  Future<void> _ask() async {
    ...
    try {
      // 若你的 ApiClient.ask 有 collectionId 參數，可再加上去
      final resp = await _api.ask(question: q, mode: _mode);
...
class _SourceTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final snippet = data['snippet'] ?? data['text'];

    // 🟢 頁碼或來源標籤
    final pageLabel = (page is num && page > 0)
        ? '第$page 頁'
        : (sourceName ?? '來源不明');
    ...
    return ListTile(
      ...
      // 🟩 顯示格式：「第3頁 · 相似度 0.842」
      title: Text('$pageLabel  ·  相似度 $scoreLabel'),
      // 🧹 這裡加上 cleanSnippet
      subtitle: Text(
        cleanSnippet(snippet is String ? snippet : snippet?.toString() ?? ''),
        maxLines: 3,
